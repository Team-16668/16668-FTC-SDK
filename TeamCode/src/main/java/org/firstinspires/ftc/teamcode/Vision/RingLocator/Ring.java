...
import static org.firstinspires.ftc.teamcode.Vision.RingLocator.RingLocator.*;
import static org.firstinspires.ftc.teamcode.Vision.RingLocator.RingLocator.maxX;
import static org.firstinspires.ftc.teamcode.Vision.RingLocator.RingLocator.maxY;
import static org.firstinspires.ftc.teamcode.Vision.RingLocator.RingLocator.minX;
import static org.firstinspires.ftc.teamcode.Vision.RingLocator.RingLocator.minY;
...
public class Ring {
    ...
    public Ring(double startX, double startY, double theta, double vx, double vy, double omega, double startTime) {
        this.startX = startX + 6.5 * Math.sin(theta);
        this.startY = startY - 6.5 * Math.cos(theta);
        this.absX = this.startX;
        this.absY = this.startY;
        this.vx = vx - 6.5 * omega * Math.sin(theta) + 150 * Math.cos(theta + 0.08);
        this.vy = vy + 6.5 * omega * Math.cos(theta) + 150 * Math.sin(theta + 0.08);
    ...
    public static ArrayList<Ring> getRingCoords(ArrayList<Ring> rings, double minX, double minY, double maxX, double maxY, double robotX, double robotY) {
        ...
        while (i < rings.size()) {
            ...
            while (j < rings.size()) {
                org.firstinspires.ftc.teamcode.Vision.RingLocator.Ring ring = rings.get(j);
    ...
    public Ring clone() {
        return new org.firstinspires.ftc.teamcode.Vision.RingLocator.Ring(relX, relY, absX, absY);
    }
